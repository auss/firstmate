...
state/               runtime records and signals; gitignored
  ...
  eggbot.check.sh    generated eggbot context-debt poll shim and its .check-trust binding; present only after bin/fm-eggbot-ingest.sh arm; producers drop schema fm-eggbot-context-debt.v1 JSON at state/eggbot/inbox/; ingest/check commands and schema: docs/configuration.md "Eggbot context-debt ingest"
  eggbot/            runtime drop path for Grok Bot context-debt events: inbox/ for atomic producer writes, processed/<event_id> ingest receipts; written only by producers and bin/fm-eggbot-ingest.sh
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, `rovo`, and `agy` for crewmates and scouts only (agy's sole primary surface is the main-session quota handover destination in `bin/fm-primary-resource.sh`); never dispatch on an unverified adapter.
