...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, `rovo`, and `agy` for crewmates and scouts only (agy's sole primary surface is the main-session quota handover in `bin/fm-primary-resource.sh`); never dispatch on an unverified adapter.
