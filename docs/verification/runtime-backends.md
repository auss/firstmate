...
Antigravity (agy) is the one exception with evidence: on 2026-09-13 every step key across all local agy 1.2.x `transcript.jsonl` and `transcript_full.jsonl` files was `step_index`, `source`, `type`, `status`, `created_at`, `content`, `thinking`, `tool_calls`, `truncated_fields`, or `error`, with no token usage, so its context reading is pinned as `agy-no-usage` rather than estimated.
agy quota handover is destination-only, with its `--prompt-interactive` successor template and primary-home trust registration covered by fixtures in `tests/fm-primary-resource.test.sh`; no real agy successor handover has been driven.
The isolated tmux regression uses an agy-named stand-in to verify that a legacy source receipt cannot exit the session; it does not establish vendor busy-state or primary-binding support.
