...
`tests/fm-primary-resource.test.sh` carries 36781 ms from [PR 4](https://github.com/auss/firstmate/pull/4)'s portable serial shard 5 timing artifact - a complete traversal whose only failure was that suite's final assertion - and `tests/fm-primary-resource-herdr-lab-e2e.test.sh` carries the 100 ms live-gate skip weight, since its live lab path is opt-in and never runs in CI; refresh both from green-run artifacts at the next hint refresh.
Taking the slowest of several CI runs rather than a single run keeps the balance honest on a slow runner.
A script with no hint gets the conservative `PORTABLE_SERIAL_DEFAULT_WEIGHT_MS` default; the current 167-script lane has seventeen such scripts, bringing its assignment weight to 4821010 ms.
=======
