...
`bin/fm-postcompact-start.sh` is the tracked `PostCompact` registration for both harnesses. It runs the compact-source session-start path even though hook stdout is not model-visible, and marks the drain's status presentation undelivered (`FM_WAKE_PRESENTATION_UNDELIVERED=1`) so no one-shot unread-status, annotation, or outcome-backstop receipt is committed against output nobody sees; the tracked `SessionStart` hook remains the delivery channel for the digest and keeps full presentation visibility.
...
The same portable suite proves the post-compact starter preserves the one-shot status presentation for the model-visible compact channel: a PostCompact run whose stdout is discarded commits no unread-status or annotation receipt, and the compact-source session start still surfaces every unread status and still commits its own presentation.
